...
export default function ConversationChatScreen() {
  interface UsageInfo { input_tokens?: number; output_tokens?: number; cost?: number; model?: string; }
  interface Message { id: string; role: 'user' | 'assistant'; content: string; thinking?: string; usage?: UsageInfo; timestamp: Date; }
  ...
  const [conversationId, setConversationId] = useState<string | null>(conversation_id || null);
  ...
  const [isHistoryLoading, setIsHistoryLoading] = useState<boolean>(true); // default true agar loading bar muncul saat mount
  const scrollViewRef = useRef<ScrollView>(null);
  const isFetchingRef = useRef(false);

  // Fetch more messages (older) when user scrolls to top
  const fetchMore = async () => {
    ...
    try {
      ...
      const res = await fetch(`${env.API_URL}/api/v1/chat/${conversation_id}/messages?limit=${PAGE_SIZE}&offset=${messages.length}`, {
        headers: { 'Authorization': `Bearer ${token}` },
      });
      if (!res.ok) throw new Error('Failed to load more');
      const data = await res.json();
      const hydrated: Message[] = (data || []).map((m: any, idx: number) => ({
  ...
  // Fetch chat history from server on mount and when conversation_id changes
  // Fetch first page of chat history
  useEffect(() => {
    ...
    const fetchHistory = async () => {
      ...
      try {
        const token = await getAccessToken();
        const res = await fetch(`${env.API_URL}/api/v1/chat/${conversation_id}/messages?limit=${PAGE_SIZE}&offset=0`, {
          headers: { 'Authorization': `Bearer ${token}` },
        });
        if (!res.ok) throw new Error('Failed to load history');
        const data = await res.json();
        // Pastikan ambil dari data.items, dan urutkan dari paling lama ke paling baru
        const items = Array.isArray(data.items) ? data.items : [];
        const hydrated: Message[] = items.map((m: any, idx: number) => {
          let thinking, content;
          if (typeof m.content === 'string' && m.content.includes('<thinkingDiv>')) {
            const thinkingRegex = /<thinkingDiv>([\s\S]*?)<\/thinkingDiv>/;
            const match = m.content.match(thinkingRegex);
            if (match) {
              thinking = match[1].trim();
              content = m.content.replace(thinkingRegex, '').trim();
            } else {
              content = m.content;
            }
          } else {
          ...
          return {
            id: m.message_id || m.id || `${Date.now()}-${idx}`,
            role: m.role === 'assistant' ? 'assistant' : 'user',
            content: content || '',
            thinking,
            usage: undefined,
        ...
        // Urutkan dari paling lama ke paling baru (created_at ASC)
  ...
  const getAccessToken = async () => {
    ...
    catch (error) { console.error('Failed to get access token:', error); return ''; }
  };
  ...
  const handleSend = async () => {
    ...
    const userMessage: Message = { id: Date.now().toString(), role: 'user', content: inputText.trim(), timestamp: new Date() };
    ...
    try {
      const response = await fetch(`${env.API_URL}/api/v1/chat/`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'Accept': 'text/event-stream', 'Authorization': `Bearer ${await getAccessToken()}` },
        body: JSON.stringify({ message: userMessage.content, conversation_id: conversationId, llm_config: { model: selectedModel, temperature: 0.7 } }),
      });
      ...
      if (!canStream) {
        ...
        let aggregated = '';
        try { const json = JSON.parse(text); const extract = (p: any): string => (p?.content ?? p?.delta ?? p?.text ?? p?.message ?? p?.choices?.[0]?.delta?.content ?? ''); aggregated = extract(json) || String(json); }
      ...
      let buffer = ''; let aggregated = ''; let newConversationId = conversationId;
      while (true) {
        ...
        for (const rawChunk of chunks) {
          ...
          if (isSSE) { const lines = rawChunk.split(/\r?\n/).filter(Boolean); for (const line of lines) { if (!line.startsWith('data: ')) continue; const data = line.slice(6).trim(); if (!data || data === '[DONE]') continue; processEvent(data); } }
          else { processEvent(rawChunk); }
        }
      }
      function processEvent(jsonLine: string) {
        try {
          ...
          if (cid && !newConversationId) { newConversationId = cid; setConversationId(newConversationId); router.replace(`/(app)/(tabs)/chat/${newConversationId}`); }
          ...
          const extract = (p: any): string => { if (!p) return ''; if (p.type === 'token_chunk' && typeof p.payload === 'string') return p.payload as string; return p?.content ?? p?.delta ?? p?.text ?? p?.message ?? p?.choices?.[0]?.delta?.content ?? ''; };
          const piece = extract(payload);
          if (piece) {
            aggregated += String(piece);
            const { thinking, content, isThinkingActive } = parseStreamingText(aggregated);
            setMessages(prev => prev.map(m => m.id === assistantMsgId ? { ...m, content, thinking } : m));
            if (isThinkingActive) setShowThinking(prev => ({ ...prev, [assistantMsgId]: true }));
            else if (thinking && !isThinkingActive && content) setShowThinking(prev => ({ ...prev, [assistantMsgId]: false }));
          }
        } catch { }
      }
    } catch (error) {
      ...
      <View style={styles.modelBar}>
        ...
        {isHistoryLoading && (
          <View style={styles.loadingBar}>
            ...
            <Text style={styles.loadingBarText}>Memuat riwayat chat...</Text>
          </View>
        )}
      </View>
      <ScrollView
        ...
        onScroll={async (e) => {
          if (e.nativeEvent.contentOffset.y <= 0 && hasMore && !isFetchingMore && !isHistoryLoading) {
            await fetchMore();
          }
        }}
      ...
      >
        {isHistoryLoading ? (
          <View style={styles.emptyState}>
            <ActivityIndicator size='large' color='#71767b' />
            <Text style={styles.emptyTitle}>Memuat riwayat chat...</Text>
          </View>
        ) : messages.length === 0 ? (
          <View style={styles.emptyState}>
          ...
          <>
            {isFetchingMore && (
              <View style={{alignItems:'center',padding:8}}>
                ...
                <Text style={{color:'#71767b',fontSize:12}}>Memuat chat lama...</Text>
              </View>
            )}
                ...
                {message.thinking && message.role === 'assistant' && (
                  <View style={styles.thinkingContainer}>
                    ...
                    {showThinking[message.id] && (
                      <View style={styles.thinkingContent}><MarkdownText style={styles.thinkingText}>{message.thinking}</MarkdownText></View>
                    )}
                  </View>
                )}
...
const styles = StyleSheet.create({
  ...
  loadingBar: { flexDirection: 'row', alignItems: 'center', gap: 8, paddingVertical: 4, paddingHorizontal: 12, backgroundColor: 'rgba(29,155,240,0.07)' },
  loadingBarText: { color: '#1d9bf0', fontSize: 13, marginLeft: 6 },
});