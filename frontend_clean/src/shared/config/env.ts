...
// - Native: jika RAW_API_URL mengandung "localhost" dan tersedia RAW_API_URL_LAN, gunakan itu
let API_URL = RAW_API_URL;
if (Platform.OS !== "web") {
  const isLocalhost = /(^|\/)localhost(?::\d+)?\/?/i.test(RAW_API_URL) || RAW_API_URL.includes("127.0.0.1");
